...
void handleSttRequest(int clientSock, ParsedHttpRequest request) {
    @autoreleasepool {
        if (!request.multipartBoundary) {
            sendErrorResponse(clientSock, 415, @"Unsupported Media Type", @"Requires multipart/form-data");
            close(clientSock);
            return;
        }
        NSString *language = DEFAULT_LANGUAGE;
        NSData *audioData = nil;
        NSString *audioFilename = @"unknown";
        for (MultipartPart *part in request.multipartParts) {
            if ([part.name isEqualToString:@"language"] && part.body.length > 0) {
                language = [[NSString alloc] initWithData:part.body encoding:NSUTF8StringEncoding] ?: language;
            } else if ([part.name isEqualToString:@"audio"] && part.body.length > 0) {
                audioData = part.body;
                audioFilename = part.filename ?: audioFilename;
            }
        }
        if (!audioData || audioData.length == 0) {
            sendErrorResponse(clientSock, 400, @"Bad Request", @"Missing or empty 'audio' part");
            close(clientSock);
            return;
        }
        NSLocale* locale = [NSLocale localeWithLocaleIdentifier:language];
        if (!locale) {
            sendErrorResponse(clientSock, 400, @"Bad Request", [NSString stringWithFormat:@"Invalid language code: %@", language]);
            close(clientSock);
            return;
        }
        ...
        int capturedClientSock = clientSock;
        NSString *capturedLanguage = [language copy];
        NSObject *sttLock = [NSObject new];
        __block BOOL sttCompletedOrTimedOut = NO;
        __block SFSpeechRecognizer* recognizer = nil;
        __block SFSpeechRecognitionTask *recTask = nil;
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(STT_TIMEOUT_SECONDS * NSEC_PER_SEC)), dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            SFSpeechRecognitionTask *taskToCancel = nil;
            @synchronized (sttLock) {
                if (!sttCompletedOrTimedOut) {
                    sttCompletedOrTimedOut = YES;
                    shouldCleanupAndClose = YES;
                    taskToCancel = recTask;
                    NSLog(@"[%d] STT Request timed out after %.f seconds.", capturedClientSock, STT_TIMEOUT_SECONDS);
                    sendErrorResponse(capturedClientSock, 504, @"Gateway Timeout", @"Speech recognition timed out");
                }
            }
            if (shouldCleanupAndClose) {
                ...
                close(capturedClientSock);
                NSLog(@"[%d] Closed socket on STT timeout.", capturedClientSock);
                recognizer = nil;
                recTask = nil;
            }
        });
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!recognizer) {
                NSLog(@"[%d] Recognizer init failed (main thread).", capturedClientSock);
                BOOL sc = NO;
                @synchronized(sttLock){
                    if(!sttCompletedOrTimedOut){
                        sttCompletedOrTimedOut = YES;
                        sc = YES;
                        sendErrorResponse(capturedClientSock, 500, @"Internal Server Error", @"Recognizer init failed");
                    }
                }
                if(sc) close(capturedClientSock);
                return;
            }
            if (!recognizer.isAvailable) {
                NSLog(@"[%d] Recognizer not available (main thread).", capturedClientSock);
                BOOL sc = NO;
                @synchronized(sttLock){
                    if(!sttCompletedOrTimedOut){
                        sttCompletedOrTimedOut = YES;
                        sc = YES;
                        sendErrorResponse(capturedClientSock, 503, @"Service Unavailable", @"Recognizer not available");
                    }
                }
                if(sc) close(capturedClientSock);
                recognizer = nil;
            ...
            if (!recReq) {
                NSLog(@"[%d] Failed to create SFSpeechRequest (main thread).", capturedClientSock);
                BOOL sc = NO;
                @synchronized(sttLock){
                    if(!sttCompletedOrTimedOut){
                        sttCompletedOrTimedOut = YES;
                        sc = YES;
                        sendErrorResponse(capturedClientSock, 500, @"Internal Server Error", @"Failed to create recognition request");
                    }
                }
                if(sc) close(capturedClientSock);
                recognizer = nil;
            ...
            recTask = [recognizer recognitionTaskWithRequest:recReq resultHandler:^(SFSpeechRecognitionResult *res, NSError *err) {
                BOOL isFinal = NO;
                NSString* finalTranscript = nil;
                NSError* recognitionError = nil;
                if (err) {
                    recognitionError = err;
                    isFinal = YES;
                } else if (res && res.isFinal) {
                    finalTranscript = res.bestTranscription.formattedString;
                    isFinal = YES;
                } else if (!res && !err && recTask.state == SFSpeechRecognitionTaskStateCompleted) {
                    isFinal = YES;
                    finalTranscript = @"";
                    NSLog(@"[%d] STT Task completed without result/error.", capturedClientSock);
                }
                ...
                if (isFinal) {
                    ...
                    @synchronized(sttLock) {
                        if (!sttCompletedOrTimedOut) {
                            ...
                            if (recognitionError) {
                                ...
                                NSString* errorMsg = [NSString stringWithFormat:@"Recognition failed: %@", recognitionError.localizedDescription];
                                int errCode = 500;
                                NSString* errDesc = @"Internal Server Error";
                                if ([recognitionError.domain isEqualToString:SFSpeechErrorDomain]) {
                                    ...
                                    if (recognitionError.code == 203){errCode = 400; errDesc = @"Bad Request"; errorMsg = @"Recog failed: No speech detected.";}
                                    else if (recognitionError.code == 209){errCode = 400; errDesc = @"Bad Request"; errorMsg = @"Recog failed: Audio format incompatible?";}
                                    else if (recognitionError.code == 1700){errCode = 503; errDesc = @"Service Unavailable"; errorMsg = @"Recog failed: Apple Speech service issue.";}
                                }
                                else if (recognitionError.code == NSUserCancelledError && [recognitionError.domain isEqualToString:NSCocoaErrorDomain]) {
                                    shouldCleanupAndClose = NO;
                                    NSLog(@"[%d] STT Task cancelled (likely by timeout).", capturedClientSock);
                                }
                    ...
                    if (shouldCleanupAndClose) {
                        ...
                        close(capturedClientSock);
                        NSLog(@"[%d] Cleaned up and closed socket in STT result handler (main thread).", capturedClientSock);
                        recognizer = nil;
                        recTask = nil;
                    }
                }
